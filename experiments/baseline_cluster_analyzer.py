"""
Model Representativeness Analyzer
Analyzes model representativeness and reliability.
"""
...
class ModelRepresentativenessAnalyzer:
    ...
    def _analyze_user_representativeness(self):
        ...
        print(f"  User overlap: {len(overlap)} ({overlap_pct:.1f}%) {'OK' if overlap_pct == 0 else 'CRITICAL'}")
    ...
    def _analyze_model_behavior(self):
        ...
        with torch.no_grad():
            ...
            for batch in self._create_batches(self.train_data, batch_size=64):
                ...
                # Check if model expects temporal features (hybrid model)
                try:
                    outputs = self.model(cluster_seq, user_features)
                except TypeError:
                    # Model expects temporal features (hybrid model)
    ...
    def generate_representativeness_report(self):
        ...
        print("\n" + "="*60)
        print("REPRESENTATIVENESS REPORT SUMMARY")
        print("="*60)
        ...
        # 3. Generalization Score (placeholder - computed in _analyze_generalization)
...
def perform_representativeness_analysis(model, clusters, train_data, val_data, device):
    """Perform representativeness analysis"""
    
    analyzer = ModelRepresentativenessAnalyzer(model, clusters, train_data, val_data, device)
...
def main_with_analysis():
    """Main function with representativeness analysis"""
    
    print("LAUNCHING REPRESENTATIVENESS ANALYSIS")
    print("=" * 60)
    
    # This analyzer should be integrated into main training script
    # Call perform_representativeness_analysis(model, clusters, train_data, val_data, device)
