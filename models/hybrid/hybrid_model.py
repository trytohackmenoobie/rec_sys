...
class HybridPersonalizedGRU(nn.Module):
    ...
    def __init__(self, num_clusters, user_feature_dim, temporal_feature_dim,
        ...
        # Финальные слои (оптимизированные)
    ...
    def forward(self, cluster_seq, user_features, temporal_features=None):
        ...
        # Temporal features (если используются)
...
def preprocess_foursquare_data_hybrid(df):
    ...
    for idx, row in df.iterrows():
        ...
        # Сбалансированные пользовательские фичи (комбинируем подходы)
        user_features = [
...
def create_hybrid_clusters(poi_counts, num_clusters=12):
    ...
    # Распределяем POI (от Improved Model)
    cluster_names = list(clusters.keys())
    
    for i, poi_id in enumerate(top_pois):
        poi_freq = poi_counts[poi_id]
        
        # Используем логику от Improved Model
        if poi_freq > 500:  # Очень популярные POI
            cluster_name = cluster_names[i % 6]  # Первые 6 кластеров
        elif poi_freq > 200:  # Популярные POI
            cluster_name = cluster_names[(i + 3) % 9]  # Средние кластеры
        else:  # Редкие POI
