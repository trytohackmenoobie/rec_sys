...
class ClusterPersonalizedGRU(nn.Module):
    """Simplified GRU model for POI cluster work"""
    ...
    def __init__(self, num_clusters, user_feature_dim, cluster_embed_dim=32, 
        ...
        # Cluster embeddings (much smaller dimension!)
...
def analyze_poi_patterns(training_data):
    """Analyze POI visit patterns for cluster creation"""
    print("Analyzing POI visit patterns...")
    
    # Собираем статистику по POI
    poi_counts = Counter()
...
def create_semantic_clusters(poi_counts, num_clusters=15):
    ...
    # In real project, this would be analysis of names, categories, POI descriptions
    
    clusters = {
        'food_dining': [],      # 0 - Еда и рестораны
        'shopping_retail': [],  # 1 - Покупки и магазины  
        'entertainment': [],    # 2 - Развлечения
        'transportation': [],   # 3 - Транспорт
        'landmarks_parks': [],  # 4 - Достопримечательности и парки
        'business_work': [],    # 5 - Бизнес и работа
        'health_fitness': [],   # 6 - Здоровье и фитнес
        'education': [],        # 7 - Образование
        'nightlife': [],        # 8 - Ночная жизнь
        'residential': [],      # 9 - Жилые районы
        'cultural': [],         # 10 - Культура и искусство
        'outdoor': [],          # 11 - Активный отдых
        'services': [],         # 12 - Услуги
        'technology': [],       # 13 - Технологии
        'miscellaneous': []     # 14 - Разное
    }
    
    # Distribute POIs by clusters (simplified logic)
