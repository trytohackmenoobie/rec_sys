...
class HybridPersonalizedGRU(nn.Module):
    ...
    def __init__(self, num_clusters, user_feature_dim, temporal_feature_dim,
        ...
        # Cluster embeddings (from Improved Model)
        self.cluster_embedding = nn.Embedding(num_clusters, cluster_embed_dim)
        
        # Temporal features (from Ultimate Model, but simplified)
        ...
        # User features (combining approaches)
...
def preprocess_foursquare_data_hybrid(df):
    ...
    for idx, row in df.iterrows():
        ...
        # Balanced user features (combining approaches)
        user_features = [
...
def create_hybrid_clusters(poi_counts, num_clusters=12):
    ...
    # Use optimal cluster size from Improved Model
    top_pois = [poi_id for poi_id, count in poi_counts.most_common(600)]  # 600 POI as in Improved Model
    
    # Create balanced clusters (from Improved Model)
    clusters = {
        'food_dining': [],        # 0 - Food and restaurants
        'shopping_retail': [],    # 1 - Shopping and stores  
        'entertainment': [],      # 2 - Entertainment
        'transportation': [],     # 3 - Transportation
        'landmarks_parks': [],    # 4 - Landmarks
    ...
    # Distribute POIs (from Improved Model)
    ...
    for i, poi_id in enumerate(top_pois):
        ...
        if poi_freq > 500:  # Very popular POIs
            cluster_name = cluster_names[i % 6]  # First 6 clusters
        elif poi_freq > 200:  # Popular POIs
...
def train_hybrid_model():
    ...
    # Training loop
    epochs = 30  # Optimal number of epochs
